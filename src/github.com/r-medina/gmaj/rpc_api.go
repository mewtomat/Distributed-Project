...
// setNextSuccessorRPC sets the next_successor ID of a remote node.
func (node *Node) setNextSuccessorRPC(remoteNode, newSucc *gmajpb.Node) error {
	client, err := node.getChordClient(remoteNode)
	if err != nil {
		return err
	}

	_, err = client.SetNextSuccessor(context.Background(), newSucc)
	return err
}
...
func (node *Node) putKeyValRPC(remoteNode *gmajpb.Node, key string, val []byte, ver int32) (bool, error) {
	client, err := node.getChordClient(remoteNode)
	if err != nil {
		return false, err
	}

	vote, err1 := client.PutKeyVal(context.Background(), &gmajpb.KeyVal{Key: key, Val: val, Ver: ver})
	return vote.Vote, err1
}
