...
type Node struct {
	*gmajpb.Node
	...
	next_successor *gmajpb.Node // This Node's second successor
	nsuccMtx   sync.RWMutex
	...
	tempstore map[string][]byte // temp val
	tsMtx     sync.RWMutex
...
func NewNode(parent *gmajpb.Node, opts ...NodeOption) (*Node, error) {
	...
	node.tempstore = make(map[string][]byte)
...
func (node *Node) join(other *gmajpb.Node) error {
	...
	nsucc, err1 := node.getSuccessorRPC(succ)
	if err1 != nil {
		return err1
	}
	node.nsuccMtx.Lock()
	node.next_successor = nsucc
	node.nsuccMtx.Unlock()
...
func (node *Node) stabilize() {
	...
	if succ == nil || err != nil {
		...
		nsucc := node.next_successor
		node.predMtx.RUnlock()
		node.nsuccMtx.RUnlock()

		nnsucc, err := node.getSuccessorRPC(nsucc)
		if err != nil {
			return
		}

		_ = node.setNextSuccessorRPC(pred, nsucc)
		_ = node.setPredecessorRPC(nsucc, node.Node)
		
		node.nsuccMtx.RLock()
		node.next_successor = nnsucc
		node.nsuccMtx.RUnlock()

		node.succMtx.RLock()
		node.successor = nsucc
		node.succMtx.RUnlock()
	...
	// node.nsuccMtx.Lock()
	// _nsucc := node.next_successor
	// node.nsuccMtx.Unlock()
	

	nsucc, _ := node.getSuccessorRPC(succ)
	
	// if nsucc.Id != nil && between(nsucc.Id, succ.Id, _nsucc.Id) {
		node.nsuccMtx.Lock()
		node.next_successor = nsucc
		node.nsuccMtx.Unlock()
	// }	
...
func (node *Node) Shutdown() {
	...
	npred, err := node.getPredecessorRPC(pred)
	if err != nil {
		return
	}
	...
	if node.Addr != succ.Addr && pred != nil {
		...
		_ = node.setNextSuccessorRPC(npred, succ)
	}
...
func (node *Node) String() string {
	...
	node.nsuccMtx.RLock()
	if node.next_successor != nil {
		nsucc = node.next_successor.Id
	}
	node.nsuccMtx.RUnlock()
	...
	return fmt.Sprintf(
		"Node-%v: Address: %s {succ:%v, nsucc:%v, pred:%v}",
