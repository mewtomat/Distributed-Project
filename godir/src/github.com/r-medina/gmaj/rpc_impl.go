...
// SetNextSuccessor sets the next_successor on the node.
func (node *Node) SetNextSuccessor(
	ctx context.Context, succ *gmajpb.Node,
) (*gmajpb.MT, error) {
	node.nsuccMtx.Lock()
	node.next_successor = succ
...
func (node *Node) PutKeyVal(ctx context.Context, kv *gmajpb.KeyVal) (*gmajpb.Vote, error) {
	vote, err := node.putKeyVal(kv)
	if err != nil {
		return nil, err
	}

	return &gmajpb.Vote{Vote: vote}, nil
}
